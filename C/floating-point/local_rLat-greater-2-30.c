...
axiomatic real_function {
  logic real rLatr (int i, int awb_lats, int yz) =
    ...
    \let j1r    = ((awb_lats - 0x1.0p+32) * (60 - i) - (yz - 0x1.0p+16)*0x1.0p+15) * 0x1.0p-32;
    \let jr     = \floor(j1r);
...
/*@
  requires 0 <= i <= 1;
  requires 1073741824 < awb_lats <= 4294967295;
...
fp rLatf_g_2_30 (int i, unsigned int awb_lats, int yz) {
  ...
  fp j1f = ((awb_lats - 0x1.0p+32) * (60.0 - i) - (yz - 0x1.0p+16)*0x1.0p+15) * 0x1.0p-32;
  fp jf = floor(j1f);
  /*@ assert j1f:
      j1f == ((awb_lats - 0x1.0p+32) * (60 - i) - (yz - 0x1.0p+16)*0x1.0p+15) * 0x1.0p-32; @*/
