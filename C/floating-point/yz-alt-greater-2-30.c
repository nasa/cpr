...
axiomatic real_function {
  logic real yzr(int i, int awb_lat) =
    \let xr = 60.0 - i;
    \let tmp1r = ((awb_lat - 0x1.0p+32) * xr + 0x1.0p+14) * 0x1.0p-15;  
    \let tmp2r = ((awb_lat - 0x1.0p+32) * xr + 0x1.0p+14) * 0x1.0p-32;  
...
/*@
  requires 0 <= i <= 1;
  requires awb_lat > 1073741824 && awb_lat <= 4294967296;
  requires \floor(i) == i;
  requires \floor(awb_lat) == awb_lat;
  ensures \result == yzr(i, awb_lat) ;
  @*/
fp yzf_g_2_30 (int i, unsigned int awb_lat) {
  fp xf    = 60.0 - i;
  /*@ assert xf:
    xf == (60.0 - i);
    @*/
  fp tmp1f = ((awb_lat - 0x1.0p+32) * xf + 0x1.0p+14) * 0x1.0p-15; 
  /*@ assert tmp1f:
    \let xr = (60.0 - i);
      tmp1f == ((awb_lat - 0x1.0p+32) * xr + 0x1.0p+14) * 0x1.0p-15;  
    @*/
  fp tmp2f = ((awb_lat - 0x1.0p+32) * xf + 0x1.0p+14) * 0x1.0p-32; 
  /*@ assert tmp2f:
    \let xr = (60.0 - i);
      tmp2f == ((awb_lat - 0x1.0p+32) * xr + 0x1.0p+14) * 0x1.0p-32;  
    @*/
