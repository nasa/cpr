...
#define __lon_enc_bound__(LAT0,LAT1) (nl_awb(lat2awb(LAT0))==1?180:(__dlon__(1,LAT1)-__dlon__(0,LAT0))/2.0-__dlon__(1,LAT1)/P2_NB)
#define __longitude_correctly_decoded__(LON0,LON1,EPS) (__mod360__(LON0-LON1) <= EPS) || (__mod360__(LON0-LON1) >= 360 - EPS)
...
int main(int argc, char * argv[]){
    ...
    while( fgets ( line, INPUT_LINE_MAX_LENGTH, fp ) != NULL ) {
        ...
        if(evn_awb_dist > awb_bound_i0) {
            printf("EVEN recovered: %.20f and actual: %.20f at %u (vs. %u)\n",deg_dec_evn_lat,deg_evn_lat,evn_awb_dist,awb_bound_i0);
            result = 1; // ERROR
        }
        ...
        if(odd_awb_dist > awb_bound_i1) {
            printf("ODD recovered: %.20f and actual: %.20f at %u (vs. %u)\n",deg_dec_odd_lat,deg_odd_lat,odd_awb_dist,awb_bound_i1);
            result = 1; // ERROR
        }

        int_type awb_lon_bound_evn = lon2awb(__lon_enc_bound__(deg_dec_evn_lat,deg_dec_odd_lat));
        ...
        int_type awb_lon_bound_odd = lon2awb(__lon_enc_bound__(deg_dec_odd_lat,deg_dec_odd_lat));
